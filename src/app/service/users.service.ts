...
export class UsersService {
  ...
  getUserUsingSubjectBehaviour(): Observable<User | null> {
    const current = this.$loggedInUser.getValue();

    if (current) {
      return of(current);
    } else {
      return this.getUserInfoByToken().pipe(
        tap((user: any) => this.$loggedInUser.next(user))
      );
    }
  }
